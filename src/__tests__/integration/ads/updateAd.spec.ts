...
describe("Test to update one ad ", () => {
  ...
  test("PATCH - Must be able to update an ad", async () => {
    ...
    const res = await request(app)
      .patch(`/vehicles/${ad.body.id}`)
      .send(mockedAdUpdated)
      .set({ Authorization: `Bearer ${token}` });

    expect(res.status).toBe(200);
    expect(res.body).toHaveProperty("message");
    expect(res.body.message).toEqual("Updated ad");
  });

  test("ERROR - Not able to update an ad with invalid uuid", async () => {
    const res = await request(app)
      .patch(`/vehicles/1`)
      .send(mockedAdUpdated)
      .set({ Authorization: `Bearer ${token}` });
  ...
  test("ERROR - Not able to update a non-existent ad", async () => {
    const res = await request(app)
      .patch(`/vehicles/c6e5958c-f8b5-4a3a-8a4a-908089a3c8b2`)
      .send(mockedAdUpdated)
      .set({ Authorization: `Bearer ${token}` });
  ...
  test("ERROR - Unable to update an ad that you do not owner", async () => {
